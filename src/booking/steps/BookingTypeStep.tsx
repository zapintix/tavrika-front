    ...
    <div className="booking-step-content">
      <div className="booking-choice-grid">
        ...
        >
          ...
          <span>{defaultGuestInfo.phone || (isMaxWebApp ? "Номер запросим через MAX" : "Номер не указан")}</span>
        </button>
      ...
      {bookingType === "self" && (
        <div className="booking-inline-card">
          <span className="booking-inline-card__label">Контакт для брони</span>
          <strong>{defaultGuestInfo.name}</strong>
          <span>{defaultGuestInfo.phone || "Номер телефона ещё не получен"}</span>
          ...
          {contactRequestError && <div className="booking-error-banner">{contactRequestError}</div>}
        </div>
      )}
        ...
        <div className="booking-form-grid">
          <label className="booking-field">
            ...
            <input
              ...
              onChange={(event) => onGuestInfoChange("name", event.target.value)}
